...
(function ($) {
	$.fn.formbuilder = function (options) {
		...
		var frmb_id = 'question-' + $('ul[id^=survey-]').length++;
		return this.each(function () {
			...
			var controlBox = function (target) {
					...
					select += '<option value="text">' + opts.messages.text + '</option>';
			...
			var fromJson = function (json) {
					...
					$.each(json, function(index, i) {
						...
						appendNewField(this.cssClass, values, options, this.answer_required, i['id']);
					});
				};
			// Wrapper for adding a new field
			var appendNewField = function (type, values, options, required, question_id) {
					field = '';
					field_type = type;
					...
					switch (type) {
					case 'text':
						appendTextInput(values, options, required, question_id);
						break;
					case 'textarea':
						appendTextarea(values, options, required, question_id);
						break;
					case 'checkbox':
						appendCheckboxGroup(values, options, required, question_id);
						break;
					case 'radio':
						appendRadioGroup(values, options, required, question_id);
						break;
					case 'select':
						appendSelectList(values, options, required, question_id);
						break;
					}
				};
			// single line input type="text"
			var appendTextInput = function (values, options, required, question_id) {
					field += '<label>' + opts.messages.label + '</label>';
					field += '<input class="fld-title" id="question_' + question_id + '" type="text" value="' + values + '" />';
					help = '';
					appendFieldLi(opts.messages.text, field, required, help);
				};
			// multi-line textarea
			var appendTextarea = function (values, options, required, question_id) {
					field += '<label>' + opts.messages.label + '</label>';
					field += '<input id="question_'+question_id+'" type="text" value="' + values + '" />';
					help = '';
					appendFieldLi(opts.messages.paragraph_field, field, required, help);
				};
			// adds a checkbox element
			var appendCheckboxGroup = function (values, options, required, question_id) {
					var title = '';
					...
					field += '<input id="question_'+question_id+'" type="text" name="title" value="' + title + '" /></div>';
			...
			var checkboxFieldHtml = function (values) {
					...
					field += '<input id="choice_'+values[2]+'" type="text" value="' + value + '" />';
					field += '<a href="#" class="remove" title="' + opts.messages.remove_message + '">' + opts.messages.remove + '</a>';
					field += '</div>';
					return field;
				};
			// adds a radio element
			var appendRadioGroup = function (values, options, required, question_id) {
					var title = '';
					if (typeof (options) === 'object') {
						title = options[0];
					}
			...
			var appendSelectList = function (values, options, required, question_id) {
					var multiple = false;
					var title = '';
					if (typeof (options) === 'object') {
						title = options[0];
			...
			$('.add_ck').live('click', function () {
				...
				$(this).parent().before(checkboxFieldHtml(values));
				return false;
			});
			...
			var save = function () {
				//console.log($(ul_obj).serializeFormList() + "&form_id=" + form_db_id + "&survey_id="+$('#survey_id').val());
					if (opts.save_url || opts.update_url) {
						has_q = $("#survey_has_questions").val();
						$.ajax({
							type: "POST",
							url: (has_q == "true") ? opts.update_url : opts.save_url,
							data: $(ul_obj).serializeFormList() + "&form_id=" + form_db_id + "&survey_id=" + $("#survey_id").val()+"&survey_has_questions="+$("#survey_has_questions").val(),
							success: function () {}
						});
					}
				};
							...
							$('#' + $(this).attr('id') + ' input[type=text]').each(function () {
								...
								if(typeof($(this).attr('id')) === 'string' ) {
									if ( $(this).attr('id').indexOf('question_') != -1 ) {
										qid = $(this).attr('id').replace("question_", "");
										serialStr += opts.prepend + '[' + li_count + '][question_id]=' +qid;
									} else if($(this).attr('id').indexOf("choice_") != -1) {
